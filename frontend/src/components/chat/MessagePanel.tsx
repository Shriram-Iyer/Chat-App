...
interface MessagePanelProps {
  ...
  onSend: (text: string, image?: File, video?: File) => void;
  draft: string;
  setDraft: (v: string) => void;
}

// Right-hand conversation area: header, messages, and composer
export const MessagePanel: React.FC<MessagePanelProps> = ({ messages, activeFriendId, activeFriendName, activeFriendStatus, onSend, draft, setDraft }) => {
                ...
                {selectedImage && (
                  <div className="relative group">
                    <div className="w-20 h-20 rounded-md overflow-hidden bg-base-200 flex items-center justify-center">
                      {/* eslint-disable-next-line @next/next/no-img-element */}
                      <img src={URL.createObjectURL(selectedImage)} alt={selectedImage.name} className="object-cover w-full h-full" />
                    </div>
                    <button type="button" onClick={removeImage} className="absolute -top-2 -right-2 btn btn-xs btn-circle btn-error opacity-0 group-hover:opacity-100 transition-opacity">×</button>
                  </div>
                )}
                {selectedVideo && (
                  <div className="relative group">
                    ...
                    <button type="button" onClick={removeVideo} className="absolute -top-2 -right-2 btn btn-xs btn-circle btn-error opacity-0 group-hover:opacity-100 transition-opacity">×</button>
                  </div>
                )}
              </div>
            )}
            <div className="flex items-end gap-2">
              ...
              <div className="dropdown dropdown-top">
                ...
                <ul tabIndex={0} className="dropdown-content menu bg-base-100 rounded-box z-[1] w-52 p-2 shadow border border-base-300">
                  <li>
                    <button onClick={handlePickImage} disabled={!!selectedImage} className="flex items-center gap-2">
                      <svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><rect width="18" height="18" x="3" y="3" rx="2" ry="2"/><circle cx="9" cy="9" r="2"/><path d="m21 15-3.086-3.086a2 2 0 0 0-2.828 0L6 21"/></svg>
                      Image
                    </button>
                  </li>
                  <li>
                    <button onClick={handlePickVideo} disabled={!!selectedVideo} className="flex items-center gap-2">
